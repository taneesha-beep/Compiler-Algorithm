...
Value Interpreter::evaluate(Node node)
{
    ...
    if (const IdentifierNode *identifier = tryAs<IdentifierNode>(node))
    {
        ...
        // until ablation D and was every bit as unreachable as the paragraphs
        // above say. It survived that long for two reasons, and D spends the
        ...
        // invariant is checked on every variable access of every test on both
        // compilers, and ablation D pays nothing for it.
        //
        ...
        // a slot past the end of the frame — a frame numbered or sized
        // wrongly, which the map could not have noticed at all, and which is
        // precisely the failure mode a slot-indexed environment newly has.
...
Value Interpreter::callFunction(const CallNode &call)
{
    ...
    // what ablation D spends. The resolver does not hand a slot back when a
    ...
    // value-initialised to `{Int, 0}`. A slot read before its assignment would
    // see the integer 0 rather than an indeterminate union member, which is
    // undefined behaviour the sanitizer build would be right to trap. No
    // program can do it — the paragraphs above the `IdentifierNode` arm are
    // that argument — so this is the belt beside that brace.
    ...
    // sites ablation D had to move. A version that indexed the two `evaluate`
    // and `executeStatement` sites and still bound arguments by name would
    // have left a string-keyed insertion on the call path of every recursive
...
//
// Ablation D made that sentence load-bearing rather than merely true. The
...
// for the stack of scopes the resolver walked. `declareOrBind` in
...
// there is an environment per call, on a stack. Since ablation D that
...
// `interpreter.h` for what D removed and what it deliberately did not touch.
//
...
Flow Interpreter::executeStatement(const Node &statement)
{
    if (const AssignNode *assign = tryAs<AssignNode>(statement))
    {
        ...
        // the resolver assigned. The frame is read back *after* the evaluation
        // for the reason above — a reference taken before it could be left
        // dangling by a call that pushed frames. The assert is the one above
        // the `IdentifierNode` arm, for the reasons argued there.
...
void Interpreter::execute(const std::vector<Node> &statements, int frameSize)
{
    ...
    // ablation D is what spends it. A top-level program that declares nothing
