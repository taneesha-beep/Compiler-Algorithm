...
// an assignment target, and since item 1.4 a parameter. **Since ablation D the
// interpreter reads all three.** Before it nothing did: the interpreter went on
// looking each name up in a `std::map<std::string, Value>`, which is the
// ordered-map lookup D exists to remove, and D is the commit that spent the
// slots by replacing that map with a vector indexed directly. Writing them here
// and reading them there was deliberate — a resolver that had also switched the
// environment over would have performed the ablation with nothing measuring it.
// See the note in `src/ast.h` on `IdentifierNode::slot`.
//
