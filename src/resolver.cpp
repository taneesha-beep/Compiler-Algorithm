...
// and one that item 3.4's per-call frame vector has no room for. Everything a
// function needs, it is passed.
//
...
struct Resolver
{
    ...
    // to assign; and item 3.4's environment, sized once from the frame and
    // never resized, would have live slots aliasing dead ones.
    //
    // Since item 3.4 this is load-bearing rather than a preference. That
    ...
    // is that they were given different numbers here. The note above
    // `executeStatement` in `src/interpreter.cpp` is the other half of this
    // sentence.
